    ...
    public partial class AddBooks : Form
    {
        ...
        private void BtnSave_Click(object sender, EventArgs e)
        {
            if (TBBookAuthor.Text != "" && TBBookName.Text != "" && TBPrice.Text != "" && TBPublication.Text != "" && TBStock.Text != "" && IsAlpha()&&IsNumberic())
            {
                ...
                cmd.Connection = con;
                con.Open();
                cmd.CommandText = "insert into BooksTbl (BookName,BookAuthor,BookPublication,BookPublishDate,BookQuantity,BookPrice) values('" + TBBookName.Text + "','" + TBBookAuthor.Text + "','" + TBPublication.Text + "','" + DateTime.Parse(DTPPublish.Text) + "'," + TBPrice.Text + "," + TBStock.Text + ")";
                cmd.ExecuteNonQuery();
                con.Close();
